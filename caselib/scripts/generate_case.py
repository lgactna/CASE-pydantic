...
- The vocabulary in `caselib.case.investigation` should come from caselib.case,
  not `caselib.uco`
- eventAttribute in caselib.uco.core should be a Optional[dict[str, Any]],
  which means you should also remove the import to caselib.uco.types
"""
...
from caselib.scripts._util import generate_docstring
...
def get_target_dir() -> Path:
    """
    Get a resolved path to "/caselib".
    """
...
class Property(BaseModel):
    ...
    def generate_type_annotation(self, containing_class: str) -> str:
        ...
        # TODO: if a forward declaration is used, a Union must be used. We might
        # just want to use Union for everything to begin with. Currently, this is
        # fixed manually after generation.
        if self.is_list:
...
def generate_import_list(dependency_graph: dict[str, set[str]], namespace: str) -> str:
    ...
    if grouped_dependencies:
        imports += f"from caselib.uco import ({', '.join(sorted(grouped_dependencies.keys()))})\n"
    # for namespace, _ in grouped_dependencies.items():
    #     imports += f"from caselib.case import {namespace}\n"
    ...
    for directory in directories:
        ...
        for artifact_file in directory.glob("*/*.ttl"):
            ...
            print(
                ...
                f" {'caselib' / target_file.relative_to(get_target_dir())}"
            )
